...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    className={cn('fixed inset-0 z-50 bg-[var(--ui-overlay-backdrop)] backdrop-blur-[1px]', className)}
    ref={ref}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      className={cn(
        'fixed left-1/2 top-1/2 z-50 grid w-[min(100%-2rem,32rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl border border-border bg-popover p-6 text-popover-foreground shadow-lg',
        className,
      )}
      ref={ref}
      {...props}
