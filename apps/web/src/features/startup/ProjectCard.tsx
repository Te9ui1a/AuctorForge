...
export function ProjectCard({ project, onClick, isSelected, variant = 'recent', onContinue }: ProjectCardProps) {
  ...
  const surfaceClassName = cn(
    ...
    variant === 'recent'
      ...
      : 'rounded-[var(--radius-md)] border-border/45 bg-background/35 p-3 shadow-none hover:border-border/65 hover:bg-background/50',
    ...
    <>
      <div className="grid gap-2">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <strong className="block min-w-0 break-all text-sm font-semibold leading-5 text-foreground">{project.name}</strong>
          <Badge variant="outline" className={cn('shrink-0 rounded-full px-2 py-0.5 text-[10px] font-medium', statusMeta.className)}>
            {statusMeta.label}
          </Badge>
        </div>
      ...
      {variant === 'management' && project.rootPath ? (
        <div className="inline-flex items-start gap-2 rounded-[var(--radius-md)] border border-border/25 bg-background/20 px-2.5 py-2 text-[11px] leading-4 text-muted-foreground">
          <FolderTree className="mt-0.5 h-3 w-3 shrink-0" aria-hidden="true" />
          <span className="break-all">{project.rootPath}</span>
        </div>
      ) : null}

      {project.phase || project.coreTask ? (
        <div className="grid gap-1 rounded-[var(--radius-md)] border border-border/25 bg-background/16 px-2.5 py-2 text-[11px] leading-4 text-muted-foreground">
  ...
  if (variant === 'recent') {
    return (
      <article
        ...
        className={cn(surfaceClassName, 'flex min-h-[148px] flex-col gap-3')}
      >
        <button
          type="button"
          className="flex min-w-0 flex-1 flex-col gap-3 rounded-[var(--radius-md)] text-left outline-none focus-visible:ring-2 focus-visible:ring-ring"
          onClick={onClick}
        ...
        <Button
          variant="secondary"
          size="sm"
          className="mt-auto h-9 w-full justify-between rounded-[var(--radius-md)] px-3 text-xs shadow-none"
  ...
  return (
    <button
      ...
      data-project-variant={variant}
      data-project-density="compact"
      className={cn(surfaceClassName, 'flex min-h-[148px] flex-col gap-3 outline-none focus-visible:ring-2 focus-visible:ring-ring')}
      onClick={onClick}
    >
      {cardBody}
      <span className="mt-auto text-[11px] font-medium leading-4 text-muted-foreground">查看项目详情与维护操作</span>
    </button>
  );
}
